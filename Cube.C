...
CubeInfo::CubeInfo() : cbID(CubeInfo::null_id), rootChnkIndex(0)
{

}

CubeInfo::CubeInfo(const string& nm): name(nm), cbID(CubeInfo::null_id), rootChnkIndex(0)
{
}
...
void CubeInfo::Get_dimension_information(const string& filename)
{
	...
	if (!ifstrm){
	    	throw GeneralError(__FILE__, __LINE__, "CubeInfo::Get_dimension_information ==> Creating ifstream object failed. Cannot get information for dimensions");
    	}
	...
	LevelMember member; //object to store temporarily the member that we get from the input file

	ifstrm >> buffer; //expected word "DIMENSION"
	if (buffer != "DIMENSION")
		throw GeneralError(__FILE__, __LINE__, "CubeInfo::Get_dimension_information ==> Wrong syntax in input file. Cannot get information for dimensions");
...
// Nikos:	
	...
	maxDepth = Chunk::MIN_DEPTH + vectDim[0].get_num_of_levels() - 1;
        #ifdef DEBUGGING
              cerr << "MAXIMUM CHUNKING DEPTH = "<<maxDepth<<endl;
        #endif	
}//CubeInfo::Get_dimension_information()
...
void CubeInfo::Insert_pseudo_levels()
{
    ...
    for (vector<Dimension>::iterator iter_dim = vectDim.begin(); iter_dim != vectDim.end(); ++iter_dim)
    {
   		if (((*iter_dim).get_num_of_levels()) < 2)
   			throw GeneralError(__FILE__, __LINE__, "CubeInfo::Insert_pseudo_levels() ==> Cannot continue the insertion of pseudo levels. A dimension has only one level.");
	...
	vector<LevelMember>::iterator iter_mem; //iterator for members' vector
  			...
  			for (int i = 0; i<levels_to_add; i++)
   			{
				...
				level.set_num_of_members((*iter_lev2).get_num_of_members());
   				...
   				{
					LevelMember member; //object to temporarily store new member
					sprintf(str,"Pseudo-member %d", (j+1));
					cout << "Creating " << str << "..." << endl;
					member.set_name(str);
					//member.set_order_code(-1-j);
					member.set_order_code(LevelMember::PSEUDO_CODE); //use a constant pseudo-code for all pseudo-members
											  //nikos has made this change
	...
	/*for (vector<Dimension>::iterator iter_dimx = vectDim.begin(); iter_dimx != vectDim.end(); ++iter_dimx)
	{
			 ...
			 ++iter_levx)
		{
			...
			for (vector<LevelMember>::iterator iter_memx = (*iter_levx).get_vectMember().begin();
...
void CubeInfo::Create_member_codes()
{
    ...
    vector<LevelMember>::iterator iter_mem2;
    char str[5];
...
void CubeInfo::Print_children(vector<Dimension_Level>::iterator iter, const int levels, const int first, const int last)
{
	...
	vector<LevelMember>::iterator iter_mem = (*iter).get_vectMember().begin();
...
unsigned int Dimension_Level::get_num_of_sibling_members(string parentMbrCode)
{
	...
	for(	vector<LevelMember>::iterator mb_iter = vectMember.begin();
		mb_iter!=vectMember.end();
		++mb_iter) {
         	if((*mb_iter).get_parent_member_code() == parentMbrCode)
			mbCounter++;
	}
	//NOTE: if this is a Pseudo level then mbCounter should be 1 at his point		
	return mbCounter;	    	
}

vector<LevelMember>::const_iterator  Dimension_Level::getMbrByMemberCode(string& mbCode)
{
	for(	vector<LevelMember>::const_iterator mb_iter = vectMember.begin();
		mb_iter!=vectMember.end();
		++mb_iter) {
