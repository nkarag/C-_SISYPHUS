                        ...
                        map<ChunkID, DirEntry>& resultMap)
{
        ...
        try{
                printDiskBucketContents_SingleTreeAndCluster(dbuckp, 3);
        }
      	catch(const char* message) {
      		string msg("printResult ==> ");
      		msg += message;
      		throw msg;
      	}
...
void createTreesOfRegion(vector<CostNode*> &treesOfRegion, unsigned int maxDepth,
                                unsigned int numFacts, unsigned int numDim)
{
        // TEST CASE 1 (sub trees under 0|0)

        //create a single cost tree (the sub-tree under cell 0|0 in fig 5 of TR)
        CostNode* root = 0;
        createSingleTree(root,maxDepth, numFacts, numDim);

        //each child-tree of root will be a tree of the region
        for(vector<CostNode*>::const_iterator child_i = root->getchild().begin();
                                                child_i != root->getchild().end(); child_i++) {

                treesOfRegion.push_back(*child_i);
