...
{
	if(!root){
		throw GeneralError("CostNode::calcTreeSize ==> null pointer\n");
	}
...
//	the returned result equals the number of directory chunks under costRoot
{
	...
	if(!costRoot)
		throw GeneralError("ASSERTION1 in CostNode::countDirChunksOfTree ==> null pointer\n");

	//ASSERTION2: it has a valid depth
	if(costRoot->getchunkHdrp()->depth > maxdepth + 1 ||
						costRoot->getchunkHdrp()->depth < Chunk::MIN_DEPTH)
		throw GeneralError("ASSERTION2 in CostNode::countDirChunksOfTree ==> invalid depth\n");
...
//	the returned result equals the number of data chunks under costRoot
{
	//ASSERTION1: not a null pointer
	if(!costRoot)
		throw GeneralError("ASSERTION1 in CostNode::countDataChunksOfTree ==> null pointer\n");

	//ASSERTION2: it has a valid depth
	if(costRoot->getchunkHdrp()->depth > maxdepth + 1 ||
						costRoot->getchunkHdrp()->depth < Chunk::MIN_DEPTH)
		throw GeneralError("ASSERTION2 in CostNode::countDataChunksOfTree ==> invalid depth\n");
...
//	the returned result equals the number of chunks (dir or data) under costRoot
{
	//ASSERTION1: not a null pointer
	if(!costRoot)
		throw GeneralError("ASSERTION1 in CostNode::countChunksOfTree ==> null pointer\n");

	//ASSERTION2: it has a valid depth
	if(costRoot->getchunkHdrp()->depth > maxdepth  ||
						costRoot->getchunkHdrp()->depth < Chunk::MIN_DEPTH)
		throw GeneralError("ASSERTION2 in CostNode::countChunksOfTree ==> invalid depth\n");
...
//      array[totNumCells].
{
	...
	if(coords.cVect.empty())
		throw GeneralError("DirChunk::calcCellOffset ==> ASSERTION1: empty coord vector\n");
        ...
        for(int c = 0; c<coords.numCoords; c++){
                  ...
                  if(coords.cVect[c] == Level_Member::PSEUDO_CODE){
                        ...
                        if(hdr.vectRange[c].leftEnd != LevelRange::NULL_RANGE ||
                                ...
                                throw GeneralError("DirChunk::calcCellOffset ==> ASSERTION2: mismatch in position of pseudo levels\n");
                  }//end if
        }//end for
        ...
        if(newvectRange.size() != newcoords.numCoords || newvectRange.size() != newcoords.cVect.size())
                throw GeneralError("DirChunk::calcCellOffset ==> ASSERTION3: mismatch in no of pseudo levels\n");
	...
	for(int d=0; d<newcoords.numCoords; d++){
		...
		if(newcoords.cVect[d] < newvectRange[d].leftEnd ||
			...
			throw GeneralError("DirChunk::calcCellOffset ==> ASSERTION4: coordinate out of level range\n");
		...
		for(int dd = d+1; dd<newcoords.numCoords; dd++){
			...
			if(newvectRange[dd].rightEnd < newvectRange[dd].leftEnd)
				throw GeneralError("DirChunk::calcCellOffset ==> ASSERTION5: invalid level range\n");
			//ASSERTION6: not null range
			if(newvectRange[dd].leftEnd == LevelRange::NULL_RANGE ||
                                                newvectRange[dd].rightEnd == LevelRange::NULL_RANGE)
				throw GeneralError("DirChunk::calcCellOffset ==> ASSERTION6: NULL level range\n");
...
//	cells have been allocated and we use the bitmap bmp to locate empty cells. hdr correspond to the chunk header of the
//	data chunk in question.
// postcondition:
...
//      The flag isEmpty is set to true if the requested cell corresponds to a 0 bit in the bitmap.
{
	//ASSERTION1: not an empty vector
	if(coords.cVect.empty())
		throw GeneralError("DataChunk::(static)calcCellOffset ==> ASSERTION1: empty coord vector\n");

	//ASSERTION2: not an empty vector
	if(bmp.empty())
		throw GeneralError("DataChunk::(static)calcCellOffset ==> ASSERTION2: empty bitmap\n");
        ...
        for(int c = 0; c<coords.numCoords; c++){
                  ...
                  if(coords.cVect[c] == Level_Member::PSEUDO_CODE){
                        //then the corresponding range should be NULL
                                throw GeneralError("DataChunk::calcCellOffset ==> ASSERTION3: pseudo coord in data chunk coordinates!\n");
                  }//end if
        }//end for
	...
	if(offset > bmp.size())
		throw GeneralError("DataChunk::(static)calcCellOffset ==> ASSERTION4: wrong offset in bitmap\n");
...
//
{
        ...
        if(!newcoords.cVect.empty())
                throw GeneralError("Coordinates::excludePseudoCoords ==> ASSERTION1: non-empty vector\n");
...
//
{
        ...
        if(!newvectRange.empty())
                throw GeneralError("ChunkHeader::excludeNULLRanges ==> ASSERTION1 : non-empty vector\n");
