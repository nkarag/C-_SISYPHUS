...
public:
	...
	static void countChunksOfTree(const CostNode* const costRoot, unsigned int maxdepth, unsigned int& total);
	/**
	 * get/set
	 */
...
CostNode* createCostTree(const string& dimFile, const string& factFile, CubeInfo& cbinfo);
...
main()//int argc, char* argv[])
{
        ...
        CostNode* costRoot = createCostTree(string("lala"),string("lalo"), cbinfo);
	...
	ofstream out("test-tree");
        ...
        CostNode::countChunksOfTree(costRoot, cbinfo.getmaxDepth(), numChunks);

        cout<<"number of chunks is: "<<numChunks<<endl;
}

void CostNode::countChunksOfTree(const CostNode* const costRoot, unsigned int maxdepth, unsigned int& total)
// precondition:
