...
bool ChunkID::operator==(const ChunkID& c1, const ChunkID& c2)
{
...
void ChunkID::extractCoords(Coordinates& c)
{
	if(cid == "root")
...
void DataChunk::calculateSize(ChunkHeader* hdrp, unsigned int numfacts)
{
	...
	if(float(hdrp->rlNumCells)/float(hdrp->totNumCells) > SPARSENESS_THRSHLD){
		...
		int no_words = ::numOfwords(hdrp->totNumCells);
		hdrp->size += (hdrp->rlNumCells * entry_size) + no_words*sizeof(WORD);	
	}
}
