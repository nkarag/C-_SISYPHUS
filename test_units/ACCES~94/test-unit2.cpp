...
void descendDepth1stCostTree(const CubeInfo& cinfo,
...
void outputVectors(int maxdepth, vector<DirChunk>* dirVectp, vector<DataChunk>* dataVectp);
...
main(int argc, char* argv[])
{
	...
	BucketID bcktID;
	try {
		descendDepth1stCostTree(cbinfo,costRoot,factFile,bcktID,dirVectp,dataVectp);
	}
        ...
        outputVectors(cbinfo.getmaxDepth(), dirVectp, dataVectp);
} // end main

void outputVectors(int maxdepth, vector<DirChunk>* dirVectp, vector<DataChunk>* dataVectp)
/* since chunks are stored in two different vectors, this routine must ensure
 * to maintain the depth-first storage of the chunks.
 * This routine will serve as the prototype for implementing the depth-first case
 * of reading the chunks from the two vectors in AccessManager::CreateDiskBucket
 */
{
cout<<"dirVectp->size(): "<<dirVectp->size()<<endl;
cout<<"dataVectp->size(): "<<dataVectp->size()<<endl;
	ofstream out("output.text");
        ...
        //for each dirchunk in the vector, read stored chunks in the row until
        // you hit a (max depth-1) chunk. Then you have to continue the reading in the
        // data chunk vector, in order to get the corresponding data chunks. After that
        // reading again resumes from the dir chunk vector at the point it was left.
        for(vector<DirChunk>::const_iterator diri = dirVectp->begin();diri != dirVectp->end();diri++){
                ...
                if((*diri).gethdr().depth ==  maxdepth-1){
                        //then we have to continue reading in the DataChunk vector

                        //find all data chunks in data chunk vect with a prefix in their chunk id
                        //equal with *diri.gethdr().id
                        ...
                        for(vector<DataChunk>::const_iterator datai = dataVectp->begin();
	                        ...
	                        if( (*datai).gethdr().id.getcid().find((*diri).gethdr().id.getcid())
	                                 != string::npos ) {
